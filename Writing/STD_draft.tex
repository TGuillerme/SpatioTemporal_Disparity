...
However, in the last decades, there as been much debate whether mammals did actually radiate in response to the K-Pg event or if they started their radiation before it and were therefore not highly affected by it.
...
In this study, we look at the effect of the K-Pg event on mammalian morphological diversity (i.e. disparity) using two major incremental changes to classic disparity through time analysis.
...
Second, we use a continuous time-slicing method for measuring changes in disparity through time allowing us to specify morphological evolutionary models (either punctuated or gradual).
...
Our analyses provides a new approach to the debate around the diversification of mammals by using both living and fossil species as well as disparity rather than taxonomic diversity as a proxy for mammalian diversity. 
...
Our results strongly %TG: not sure if this adjective is nice but I think our results are strong (i.e. same results (no increase) with rarefaction or other disparity metrics).
...
% TG: A funny note (not to include in this paper): I more and more start to believe that the K-Pg extinction is a global marine extinction but on the continents, it's only a Northern-American (and also maybe European) extinction...
...
\cite{Slater2012MEE} found good support for a shift of mode of body mass evolution (from an Ornstein--Uhlenbeck to a Brownian Motion) %TG: or just: "(a significant increase in cumulative body mass variance after the K-Pg event)"
...
Additionally, it comes with no statistical caveats %TG: or "assumptions" but I think it's more like a problem.
