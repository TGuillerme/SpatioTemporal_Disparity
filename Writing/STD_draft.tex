...
% NC: OK it's not variables that need to be independent but data points. TG: are you sure? I thought the error was when measuring the variance of the PC score, it's not actually the representing the total variance of the data since it ignores the covariance part (as if you have two axis A and B, the variance for A = variance(A) + covariance(A,B)/2)
...
% NC: HANG ON! Are we using morhological disparity actually? Or cladistic disparty? Are they the same? Are you actually using genetic data too? If so that's hardly morpho disparity! Might need to rewrite the section 2 above. TG: nope, cladistic is correct. We use cladistic data only (no genetic data) to measure the changes in disparity.
