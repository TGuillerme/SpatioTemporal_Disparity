...
In fact, as previously shown, both focal taxonomic levels (i.e. family \textit{vs.} genus) reaches maximal disparity early in their history \cite{Hughes20082013}.
Additionally, the patterns of disparity seem to plateau at their maximum disparity and are clearly decoupled from taxonomic diversity \citep{slaterCetacean,ruta2013,hopkinsdecoupling2013}.
...
Finally, we found a fundamental difference with \cite{Slater2012MEE} which shows solid evidences for a change in mode of body mass evolution at the K-Pg boundary.
...
Firstly, both our data sets are limited and do not represent the full mammalian diversity, especially in the Neogene (23 Mya) where no fossils were represented in our data set.
...
Additionally \cite{Raia2012} have shown that mammalian diversification rates declined throughout the whole Cenozoic suggesting that an effect of the K-Pg boundary would be more likely detected during the Palaeogene when mammalian diversification rates where still high.
...
\cite{Cartmill2012} also argues that the use of higher taxa definition in general might be obsolete ``there is only a long, geologically slow cascade of accumulating small apomorphies''.
...
We argue that if a significant change in disparity occured at the K-Pg boundary in any of such infraclass (Placentalia, Marsupialia, etc...) it would be detectable even at a higher level (i.e. changes in Placentalia correspond by definition by changes in Eutheria and Mammaliaformes).
