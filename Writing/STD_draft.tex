...
\vspace{1in}
% TG: I removed the centre for biodiversity for gaining space
...
%
% CUT DOWN THE NUMBER OF CITES FOR THE SUBMISSION ONLY (NOT THE THESIS)
%
...
Because the effect of a mass extinction event on a group's evolution might not be detectable directly after the event due to delays in recovery \citep{chen2012timing,chen2014complete} % TG: will find that one later one FIND THE DURATION OF LAG FROM THERE
...
Disparity in the Mammaliaformes dataset reaches a plateau a plateau during the Anisian (middle Triassic; 245 Ma) and fluctuates during the rest of the Mesozoic and the Cenozoic (Fig \ref{fig:Fig_Raw_results} and Fig @@@ supplementary).
These changes in disparity do not appear to be fully linked to the number of tips and nodes used to estimate disparity  (Fig \ref{fig:Fig_Raw_results} and @@rar@@sup).
Disparity in the Eutheria dataset reaches a plateau at the end of the Jurassic (150 Ma) and in the mean time, the number of tips and nodes (i.e. a proxy for taxonomic richness) increases up to the K-Pg boundary and then decreases throughout the Cenozoic (Fig \ref{fig:Fig_Raw_results}).
Contra to the patterns observed in the Mammaliaformes dataset, disparity changes seems to be less linked to the number of tips and nodes available for estimating disparity (Fig \ref{fig:Fig_Raw_results}).
...
We do find evidence for a lag effect of the K-Pg boundary in Eutheria under a punctuated evolutionary model at 50 and 40 Ma.
However, this may reflect changes in disparity due to the Palaeocene-Eocene Thermal Maximum Event (PETM; $\sim$56 Ma) rather than a lagged effect of the K-Pg boundary
In fact, the changes are observed respectively 6 and 16 Ma after the PETM and 16 and 26 Ma after the K-Pg event.
% NC: This needs to be added in light of the new results. Bit more to add on why it's probably not the K-Pg and probably not that interesting.

Our results are similar or different to Wilson (mentioned in the intro already) - might want to add to Grossnickle.
They found this and that.
One major difference is because TEM.
However one other study use both living and fossil, and interestingly our results are fundamentally different to those of \cite{Slater2012MEE}, the dataset on which our Mammaliaformes analyses are based. 
