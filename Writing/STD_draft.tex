...
Mass extinctions result in catastrophic species richness declines. 
For example, 95\% of marine genera went extinct during the end Permian extinction\cite{RaupPT,BentonPT}, and % NC add another brief example or two. 
. 
However, the long-term effects of such events are more varied \cite{Erwin1998344}, and include species richness increases in some clades, species richness declines in others, 
% NC: check which refs belong where as I split this e.g. \cite{Benton85,Erwin1998344,fritzdiversity2013}) 
shifts in ecological dominance (e.g. \cite{Brusatte12092008,toljagictriassic-jurassic2013,bensonfaunal2014}) and changes in morphological diversity (e.g. \cite{friedmanexplosive2010,brusattedinosaur2012}). 
These effects may occur simultaneously or independently \cite{slaterCetacean,ruta2013,hopkinsdecoupling2013} relatively shortly after the mass extinction (e.g. \cite{friedmanexplosive2010,toljagictriassic-jurassic2013}). % NC: Simultaneously or independently of what???
% NC: Need a sentence here that explains why you focus on the ecological dominance side of the Q e.g.:
...
% bit more explanation in this paragraph of what you mean by ecological dominance. Is this the phrase we want to use?

% 2§ explaining the rise of the age of the mammals view.
% NC: Bit weird phrasing it as "this view" "this interpretation" and "this hypothesis". Better to be specific and also to stick to one way of describing it - is it a view, an interpretation or a hypothesis? I've gone with idea...
% NC: Secondly, remember it's one idea, one paragraph. So I see this paragraph as being about paleo views of the rise of the age of mammals. So I've moved the start of the paragraph to the end of the previous one.

One classical example of a shift in ecological dominance is the replacement of non-avian dinosaurs by placental mammals after the Cretaceous-Palaeogene (K-Pg) mass extinction 66 Million years ago (Mya) \cite{rennetime2013}, leading to the ``rise of the age of the mammals" \cite{archibald2011extinction,Lovergrove}. 
This is based on the idea that placental mammals radiated during the Palaeogene because the K-Pg extinction led to an ``ecological release from the vice grip which the dinosaurs held over Mesozoic mammals" \cite{Lovergrove}. % TG: this might be an aggressive use of the quote (I don't know if you plan to ever work with Lovergrove) but I think it could ring positively to few non-old-school people.
% NC:  probably better to write in your own words.
However, evidence for this is mixed. % NC: Need to define what "this" 
Thorough analysis of the fossil record (e.g. \cite{goswamia2011,O'Leary08022013}) supports the idea that placental mammals ecologically replaced the non avian dinosaurs, as there are very few placental mammal fossils before the K-Pg event and many afterwards \cite{archibald2011extinction,goswamia2011,Slater2012MEE,O'Leary08022013,Wilson2013,Brusatte2015}. 
This suggests that although placental mammals originated during the late Cretaceous, they only started diversifying during the Palaeogene \cite{O'Leary08022013,Springer09082013,O’Leary09082013}, 
% perhaps because they were able to fill the ``empty'' niches left by the extinction of many terrestrial vertebrates at the K-Pg boundary (including the dominant non-avian dinosaur group \cite{archibald2011extinction,O'Leary08022013,Brusatte2015}).
% NC: doesn't this just repeat what you've said previously?
%\cite{Pearman2008149,slaterCetacean,hopkinsdecoupling2013}. % NC: Not sure where these refs belong
This allowed the placental mammals to undergo an adaptive radiation 
% NC: Is it really an adaptive radiation? Tricky terminology
into these vacant niches during the Palaeogene until they rapidly reached ecological dominance \cite{archibald2011extinction,O'Leary08022013,Lovergrove}. This can be opposed to shifts in ecological dominance due to direct competition (e.g. \cite{Brusatte12092008}).
% NC I wonder if this needs expanding somewhere.

% §3 However, this is contrasted with molecular data. Also this is hard to explain due to timing/concept difficulties
Evidence from molecular data suggests that the diversification of placental mammals started prior to the K-Pg extinction event without being drastically affected by it \cite{Douady2003285,bininda2007delayed,meredithimpacts2011,Stadler12042011,beckancient2014}. 
% NC: Need some sort of sentence of "why are neo and paleo results different? Can we ever come to a consensus"

One possible reason for the discrepancies between palaeontological and neontoligcal evidence for XXX is the timing of the events around the K-Pg boundary. 
This is a crucial element for investigating the effect of the extinction event on mammals. 
Previous work has led to a consensus concerning the timing of the events around the K-Pg boundary, (asteroid impact at 66 Mya \cite{rennetime2013}) as well as the causes
% NC: causes of what?
(e.g. \cite{rennetime2013,Brusatte2015}) and the effect on species extinction (e.g. \cite{Erwin1998344,Brusatte2015}) and diversification (e.g. \cite{Stadler12042011,meredithimpacts2011,O'Leary08022013}). 
Therefore, the two hypothesis probably differs on the focal data (palaeontological or neontological) and the methods used (e.g. \cite{meredithimpacts2011} \textit{vs.} \cite{O'Leary08022013}). However, one major caveat in both approaches is the interpretation of the concept of adaptive radiation \cite{OlsonRadiation,Losos2010,glor2010phylogenetic} and niche vacancy \cite{Pearman2008149}. Most studies focus on taxonomic diversity as a proxy for niche occupancy \cite{Stadler12042011,meredithimpacts2011,O'Leary08022013} in order to validate/reject an adaptive radiation scenario \cite{archibald2011extinction,O'Leary08022013,Brusatte2015}. Therefore, to determine whether mammals underwent an adaptive radiation after the K-Pg boundary, two elements are crucial: (1) the data and (2) the proxy used to determine niche occupancy.

% §4 One can tackel the three crucial elements as follow.
Firstly, the discrepancy between the palaeontological and the neontological approach can be solved by combining all the data available (morphological data for both living and fossil taxa along with molecular data for living taxa; the Total Evidence method \cite{eernissetaxonomic1993,ronquista2012}) and using the state of the art methods in dating such as the tip-dating method \cite{ronquista2012,Wood01032013}. Secondly, adaptive radiation due to vacant ecological niches is still much debated \cite{OlsonRadiation,Losos2010,glor2010phylogenetic} but studies have successfully used taxonomic diversity \cite{Stadler12042011,meredithimpacts2011,O'Leary08022013} and/or variation in morphologies (disparity) \cite{Wills1994,BIJ:BIJ455,Wesley-Hunt2005,brusatte50,cisneros2010,prentice2011,anderson2012using,Hughes20082013,bentonmodels2014} as a way to detect changes in ecological dominance or roles.

% §5 In this study... Maybe add somewhere that we updated 20 years old methods.
In this study, we propose an updated approach to test whether mammals underwent an adaptive radiation % NC: This comes out of nowhere, are you really testing for adaptive radiation?
during the Palaeogene due to the K-Pg extinction. 
We measured the phylogenetic diversity and the the cladistic morphological disparity from two previously published studies \cite{Slater2012MEE,beckancient2014} based on both molecular and morphological data and using the tip-dating method \cite{ronquista2012,Wood01032013}. 
We measured the variation of both diversity and disparity through time using a new time-slicing approach that provides a fine grain estimation of the diversity and disparity through time and also allows to make different assumption on the evolutionary models underlying the morphological characters evolution (either constant or punctuated). 
Finally, to test whether mammals display a significant change in diversity and disparity after the K-Pg boundary, we compared the observed changes to a null model to determine whether the group underwent an adaptive radiation. 
We found that there is no significant increase in cladistic disparity after the K-Pg event and that actually the disparity in mammals piked during the K-Pg event. 
