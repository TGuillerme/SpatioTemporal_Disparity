...
% NC: De-americanize. Look for izes and replace with ises. 
...
\bigskip
% NC: I know extinction appears twice but need to be clear what the event is!
...
Our findings contradict the popular theory that the non-avian dinosaurs and other Mesozoic % NC: Should have a capital letter right?
tetrapods were restricting mammalian evolution, and that their extinction liberated ecological niches for mammals to evolve into.
%TG: currently 286 words. No limit for PLoS Biology but maybe we want to keep it under 300?
\end{abstract}

% NC: I've cut this down a fair bit so it's ready for PRSB if needed.
...
\section{Discussion}
% §1 - What did we found
%This study tested whether the extinction of many terrestrial vertebrates (including the non-avian dinosaurs) at the K-Pg extinction event influenced mammalian morphological evolution.
% NC: Not sure we need this first sentence...
...
% NC: Yeah you're right, I combined them. 
...
We did, however, detect a small, yet significant, increase in disparity during the Eocene (45 Ma) under a punctuated evolutionary model using the Eutheria dataset.
 % NC: I'm right aren't I, it's just in Eutheria? 
...
% NC: Worth redefining stuff in the discussion in case people didn't read the methods!
...
The 45 Ma time slice %TG: or subsample? Maybe standardize it by "subsample = the tips and nodes sampled a time X (i.e. tips and nodes are rarely present at time X)" and "time slice = the true biological instant photography of the species at time X"
% NC: Your choice! Later on you use subsample... Check it for the whole discussion.
 samples the long branch ($\sim$50 Ma) leading to \textit{Lepidictis} (33.9 to 33.3 Ma) that branches with its closest relative \textit{Gypsonictops} (66.8 to 66 Ma) in the early Upper Cretaceous $\sim$90 Ma (see Fig S2 @@@).
...
Incidentally, this increase can also be linked to the number of tips and nodes used in the comparison (43 versus 23 tips and nodes at respectively 70 and 45 Ma), because the increase is not significant in the rarefied analysis (see supplementary results @@@ with only eight tips and nodes).
...
It is not unlikely that mammalian disparity \citep[\textit{sensu} ba\"{u}plan; e.g.][]{Hughes20082013} % NC: Not really sure why the sensu bauplan bit is here.
increased rapidly early in their evolutionary history and then remained constant \citep[Fig \ref{fig:Fig_Raw_results};][]{Close2015,Lee2015R759} while body mass variation continued to increase, especially after K-Pg \citep{Slater2012MEE}.
...
The disparity of each subsample is necessarily dependent on disparity in the previous subsamples. % NC: Not exactly by definition...
...
Additionally, it comes with no statistical caveats %TG: or "assumptions" but I think it's more like a problem.
 unlike the sums or products of variances that should also include covariances between axes to correctly assess the exact size of the cladisto-space (even though the covariance term is usually close to 0 because of the eigen decomposition). % NC: Citation for this?
