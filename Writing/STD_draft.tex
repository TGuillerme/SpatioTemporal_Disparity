...
% Abiotic and biotic variations can have a cascade effect on biodiversity and lead to drastic changes in species richness and composition as well as changes in species ecological roles and dominant clades \citep{Brusatte12092008}. 
...
We chose these two data sets because they have a similar number of taxa and morphological characters, and both use the Total Evidence tip dating method \citep{ronquista2012} to construct their phylogenies. However, the matrices differ in their taxonomic resolution (Family-level \textit{vs.} Genus-level), taxonomic scope (Mammaliamorpha \textit{vs.} Eutheria) and temporal range (310-0 Mya \textit{vs.} 170-0 Mya). % NC: Hmmm this is kind of repeating what you said above, can you combine?

We used the first and last occurrences as the temporal span of each taxon in our analysis.% This data was collected from the original publications \citep{MEE3:MEE312084,beckancient2014}.
...
For both datasets %\citep{MEE3:MEE312084,beckancient2014}, % NC: Don't think you need to recite here, also maybe refer to them as two datasets (that each contain a matrix and a tree)
...
between two time intervals, the character state is assumed to be constant for the whole duration of the interval and then evolve quickly to reach the state in the subsequent time interval; \citealt{Gould1977}. % NC: Why is this a problem?
