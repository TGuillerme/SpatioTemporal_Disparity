...
To test this, we performed t-tests among the time subsamples before and after the K-Pg boundary 66 million years ago, i.e. the subsamples at 70 and 65 Ma \citep{anderson2012using,zelditch2012geometric,smith2014joined}% NC: What are these citations here for?
% TG: CHANGE THAT IN THE TABLES!
...
We argue that this difference can be due to the number of traits used in \cite{Slater2012MEE} (one continuous trait; body mass) and the present study (446 discrete traits; the morphological characters).
Both variation in morphology (i.e. from cladistic data) and variation in body mass are two different and aspects of diversity and can likely be decoupled in the same way as taxonomic diversity is decoupled from disparity \citep{slaterCetacean,ruta2013,hopkinsdecoupling2013}.
...
Besides, this might not cause a serious under-estimation of disparity, at least for the Mammaliaformes dataset, since their diversity peaked during the late Cretaceous \citep[Campanian; 72.1--83.6 Ma;][]{Newham201432} and that mammalian diversification rates declined throughout the whole Cenozoic \citep{Raia2012}.
Therefore, an eventual effect of the K-Pg boundary would be more likely to be detected during the Paleogene when mammlian diversity was highest.
...
Thirdly, we used a time-slicing method instead of binning the data into time intervals \citep[e.g in:][]{cisneros2010,prentice2011,Hughes20082013,hopkinsdecoupling2013,bentonmodels2014,bensonfaunal2014} thus allowing us to avoid two caveats from the later method:
\begin{enumerate}[(i)]
