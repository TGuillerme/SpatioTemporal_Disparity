...
This method (referred to as MDS; e.g. in \citealt{DonohueDim}; PCO; e.g. in \citealt{Brusatte2015}; or PCoA; e.g. \citealt{paradisape:2004}) is an eigen decomposition of the distance matrix.
Because we used Gower distances instead of raw Euclidean distances, negative eigenvalues can be calculated.
To avoid this, we first transformed the distance matrix by applying the Cailliez correction \citep{cailliez1983analytical} which multiplies the values in the distance matrix (apart from the diagonal) by a constant $c^*$ (as used in \citealt{toljagictriassic-jurassic2013}), 
% NC: and where does c* come from?
We were then able to extract $n$ eigenvectors (representing the $n$ dimensions of the cladisto-space) where $n$ is equal to $k-1$, i.e. the number of taxa in each matrix ($k$) minus the last eigenvector which is always null after applying the Cailliez correction.
