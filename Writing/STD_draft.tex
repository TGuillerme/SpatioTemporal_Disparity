...
Throughout history, life on Earth has suffered a series of catastrophic mass extinction events resulting in drastic declines in global biodiversity \citep[e.g.][]{RaupPT,BentonPT,rennetime2013,Brusatte2015}. % TG Maybe that one is better? Sudden and catastrophic contransting with long-term and varied in the second sentence?
% NC: I've removed "sudden" as there is debate about how fast these events were, and added "mass" so the two sentences are connected by the same terminology. Repetition is good if it makes the message clear. Could probably remove either "catastrophic" or "drastic" - we are writing a paper not a dramatic novel!
...
% NC: Can you be more specific - what shifts are each of these references referring to?
...
There are three main reasons why there is still debate about the timing of the diversification of placental mammals. In this paper we focus on solving these issues as follows: % NC: Needs a bit more finesse but you get the idea 
  \begin{enumerate}
    ...
    % NC: Maybe have fewer references here? Make it clear why they are here - e.g. this is used in many other studies as their measure of diversity.
...
% NC: OK it's not variables that need to be independent but data points
...
% NC: Need to be a bit more specific here. Do these papers use the methods wrongly? Or just use the old methods? Also need to briefly explain why they break these assumptions.
  ...
  \end{enumerate}
% NC: time slicing is already a thing that exists. It is what you are doing. What you've done isn't new, it's just new to paleo types.
% NC: Generally refer to "constant" as gradual evolution.
...
% NC: HANG ON! Are we using morhological disparity actually? Or cladistic disparty? Are they the same? Are you actually using genetic data too? If so that's hardly morpho disparity! Might need to rewrite the section 2 above.
