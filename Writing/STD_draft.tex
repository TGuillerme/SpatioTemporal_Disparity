...
This method (similarly refereed as MDS \citep[e.g.][]{DonohueDim}, PCA \citep[e.g.][]{finlay2015morphological} or PCO \citep[e.g.][]{Brusatte2015}) is an eigen linear transformation based of the covariance matrix from the distance matrix.
Because covariance between certain taxa in the distance matrix can be negative, we transformed it using the Cailliez correction \citep{cailliez1983analytical} which multiplies all the covariance values by a constant (e.g. in \citealt{toljagictriassic-jurassic2013}).
...
% TG: not sure about the placement of each section below 
...
%Not sure about this null model section
