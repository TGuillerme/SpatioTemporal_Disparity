...
\noindent{\Large \bf Mammalian morphological diversity does not increase in response to the extinction of the (non-avian) dinosaurs.} 
...
%TG: Rewrote this sentence as well (saying the same twice)
Finally, we tested whether mammals display significant changes in disparity between the end of the Cretaceous and throughout the Cenozoic.
%TG: novelty part:
To our knowledge, this study is the first to approach the debate of the effect of the K-Pg event on mammalian evolution using Total Evidence phylogenies and calculating disparity through time in a continuous way \citep[but see][for a similar question]{halliday2013testing}.
%In fact, we investigate this question using improved methods for measuring disparity through time in a continuous way \citep[cf.][]{brusatte50,Brusatte12092008,cisneros2010,thorneresetting2011,prentice2011,brusattedinosaur2012,toljagictriassic-jurassic2013,ruta2013,bentonmodels2014,bensonfaunal2014}.
%
This new approach allowed us to find no significant changes in mammalian disparity between the end of the Cretaceous and any time during the Cenozoic.
...
\cite{beckancient2014} ranges from 170 Ma (Middle Jurassic) to the present and focuses on eutherians at the genus-level.
...
%
Both trees contain few taxa compared to the overall living and fossil mammalian taxonomic diversity \citep{wilson2005mammal,archibald2011extinction}. %TG: or actually maybe cite the book on my desk in Dublin.
This is mainly due to the fact that Total Evidence trees need a lot of data \citep{GuillermeCooper} and especially morphological data for living taxa \citep{MissingMammals}.
Therefore, most Total Evidence studies to date contain usually one or two orders of magnitude less taxa than more common phylogenies based solely on molecular data (e.g. thousands of taxa in \citealt{bininda2007delayed,meredithimpacts2011,jetzthe2012} \textit{vs.} hundreds in \citealt{ronquista2012,Slater2012MEE,Wood01032013,beckancient2014}). %TG: or remove Jetz?
...
\cite{foth2012different} and \cite{hetherington2015cladistic} have empirically shown inter-taxon distances are not different in a morphospace or a cladisto-space.
...
This led to us removing 11 taxa from \cite{Slater2012MEE} but none from \cite{beckancient2014}.
...
% NC: It's possibly worth explicitly stating this somewhere, maybe in the introduction?
% TG: see added sentence in first paragraph
...
where:
\begin{equation}
    Centroid_{n}=\frac{\displaystyle\sum(\mathbf{v}_{n})}{k} % TG: however, for the centroid, yes, it's just the mean value of each dimension (so divided by k)
    \label{centroid}
\end{equation}

\noindent
and $\mathbf{v}_{n}$ is any of the $n$ eigenvectors (i.e. any of the $n$ dimension of the cladisto-space), $Centroid_{n}$ is the mean value of the $n^{th}$ eigenvector (equation \ref{centroid}) and $k$ is the total number of taxa.
...
We applied our time-slicing approach to the two cladisto-spaces calculated from \cite{Slater2012MEE} and \cite{beckancient2014}, time-slicing the phylogeny every five million years from 170 Ma to the present resulting in 35 subsamples of the cladisto-space.
...
We then calculated the median disparity value for each subsample along with the 50\% and the 95\% confidence intervals.
We also recorded the number of phylogenetic elements (nodes and tips) in each subsample as a proxy for representing the taxonomic diversity.
...
We can predict that if the K-Pg event had an effect on mammalian disparity, we would then expect to see a significant difference between the disparity at the end of the Cretaceous and at the start of the Paleogene.
To test that, we performed t-tests among the time subsamples before and after the K-Pg boundary \citep[respectively between 70 and 65 Ma][]{anderson2012using,zelditch2012geometric,smith2014joined} % TG: CHANGE THAT IN THE TABLES!
 for both datasets \citep{Slater2012MEE,beckancient2014} and both evolutionary models (gradual and punctuated).
Because the effect of a mass extinction event on a group's evolution might not be detectable directly after it (i.e. lag effect; CITE), we also tested if there was a significant difference in disparity between the end of the Cretaceous and any subsample during the Cenozoic.
Because the later tests involved multiple t-tests, we corrected each p-value by multiplying them by the number of comparisons \citep[Holm-Bonferonni correction;][]{holm1979simple}.
...
Therefore, to test whether our results of the effect of the K-Pg event where driven by taxonomic diversity, we reran the same t-tests (direct effect and lag effect) on both rarefied datasets.
In \cite{Slater2012MEE}, the minimum number of taxa in each subsample from 170 Ma to present was eight.
In \cite{beckancient2014}, the minimum number of taxa in each subsample was three, however, from 150 Ma until the present, the minimum number of taxa is eight.
To make both datasets comparable, we used eight as a minimum number of taxa for the rarefied bootstrap measurements, therefore in \cite{beckancient2014} we ignored the subsample between 170 and 150 Ma that only has three taxa.


%TG: change the results into one table with all models and all datasests pre-post-KT (no corrections) and then t-tests pre-post-KT (lag effect).
%TG: check if bonferonni is the best one.
...
% NC: I think the original plan was that we look at disparity through time AND the K-Pg effect. But if you want to focus on that question this is fine.
%---------------------------------------------
%
%       RESULTS
%
...
When we tested for a direct effect of the K-Pg boundary on both data sets and under both evolutionary models and found ... (Table @) %TG: to measure!
When testing for a lag effect of the K-Pg boundary, we found that ... (Table @) %TG: to measure!
...
When testing for a direct effect of the K-Pg boundary, we found ... (Table @) %TG: to measure!
Finally, when testing for a lag effect of the K-Pg boundary, we found that ... (Table @) %TG: to measure!
...
%TG: + 4 tables same format as the post-hoc tests
...
\begin{landscape}
\begin{table}[!ht]
\caption{PERMANOVA results testing the effect of time on the ordinated distance matrix using Euclidean distance with 1000 permutations. % NC: Needs a bit more explanation
Data: Eutheria \citep[data from][]{beckancient2014}; Mammaliaformes, \citep[data from][]{Slater2012MEE}. Significant effects are highlighted in bold. Model = evolutionary model; df = degrees of freedom;  *p \textless 0.05; **p \textless 0.01; ***p \textless 0.001} %Or just say three stars since it's the only results? % NC: We talked about this!!! Use the standard please and stop making up your own.
...
% NC: If they're big and near a page break it does that automatically - super annoying!
% NC: Again I think we need a quick think about these tables, the legends etc.
\begin{table}[!ht]
...
%Instead, mammalian morphological diversity increases... % NC: Is there something we can add here about when it does happen?
...
%Additionally, as showed previously, disparity display a pattern decoupled from taxonomic diversity \citep{slaterCetacean,ruta2013,hopkinsdecoupling2013}. % NC: Didn't we decide this was not true?
...
Additionally \cite{Raia2012} have shown that mammalian diversification rates declined throughout the whole Cenozoic, suggesting therefore, that an effect of the K-Pg boundary would be more likely detected during the Paleogene. %when mammalian diversification rates where still high.
...
% TG: these two last sentences are discussed in the methods
...
Code for reproducing the analysis is available on GitHub (\url{github.com/TGuillerme/SpatioTemporal_Disparity}).
