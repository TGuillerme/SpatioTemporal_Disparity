...
We compared the last time subsample before the K-Pg boundary (70 Ma) to the first subsample of the Paleocene (65 Ma) for both the Mammaliaformes and Eutheria datasets and using both the gradual and punctuated evolutionary models. %TG say somewhere that this 1Ma recovery is enough (Wilson detects changes after 400kya).
%
% DISCUSS
%
Because the effect of a mass extinction event on a group's evolution might not be detectable directly after the event due to delays in recovery \citep[e.g.][estimated that ecosystems only fully recovered 8-9 Ma after the Permo-Triassic mass extinction]{chen2012timing}, we also tested whether there was a significant difference in disparity between the end of the Cretaceous (70 Ma) and all subsamples from the Paleocene (65, 60 and 55 Ma) as well as the subsamples of the Eocene up to the Late Priabonian, 35 Ma.
% NC: Explain why you stop at 35 Ma and not 50 Ma (60 - 9 = 51 not 35). Stuff about wanting to include the PETM.
Even though comparing the disparity between the last subsample of the Cretaceous and the subsamples of the Eocene is outside the order of magnitude of recovery time proposed by \cite[$>$ 10 Ma]{chen2012timing}, the Eocene was the theatre of major faunal changes after the Paleocen-Eocene Thermal Maximum (PETM). %TG: actually I think we can just drop that no? 
%
% DISCUSS
%
...
% NC: What about this? Rule of thumb is that if the legends are the same and column headings are the same they should be the same table unless they're way too big to fit. These will be fine for the thesis if you get rid of double spacing. You may also want to highlight entire rows in grey shading if they are important - for the thesis not the paper. TG: Way better!
...
Initially, disparity increases rapidly but then reaches a plateau %during the late Triassic (Ladinian; 240 Ma) for the Mammaliaformes dataset and at the end of the Jurassic (Tithonian; 150 Ma) for Eutheria dataset.
% NC: This is in the results
after approximately 12\%@ of the evolutionary history of Mammaliaformes and 18\%@ of the evolutionary history of Eutheria.
These patterns 
% NC: which patterns? The increase followed by a plateau? Or the more precise increase til the late Triassic? Be precise.
of mammalian disparity are consistent with previous mammalian studies (\citealt{Close2015} but see \citealt{Grossnickle2013} who find a second increase in disparity during the Late Cretaceous) as well as with patterns of disparity in metazoans more generally \citep{Hughes20082013}. 
...
% TG: concerning the ages thing, what's better, using the geological names (e.g. Paleocene) or the geological ages (65-50) knowing that the later does not really match with the ages of the geological period (e.g. Paleocene is exactly 66 to 56 not 65 to 55).
% NC: Not sure really, I guess be consistent. Always good to have some dates there for those of us who can't ever remember the order of geological periods! Reviewers may ask for changes.
...
% NC: I still think it needs a little more careful explanation. I feel you've rushed dealing with these comments.

% NC: This needs to be carefully rewritten with more logic. What did you find in your results, what do the results suggest. You've gone right to the suggestions missing a logical step that you need to convince the reader.

Our results differ from previous studies 
% NC: studies or study?
that found an increase in disparity in the Early Paleocene % NC: date? 
\citep{Wilson2013}. %TG: actually I dropped Grossnickle, they don't test K-Pg effect and just detect increase during Cretaceous, we don't care about that. % NC: OK
There are several possible explanations for these differences including: (1) the use of locally constraint % NC: what are locally constraint data?
dental morphometric data \textit{versus} global dental, cranial and post-cranial characters; (2) using biostratigraphic time bins \textit{versus} continuous time subsamples; or (3) using palaeontological data only \textit{versus} using Total Evidence trees \citep[][\textit{versus} this study]{Wilson2013}.
% NC: Numbered lists are not always your friend. Write this out in full and explain more clearly. It's not completely clear which is your study.
Also, it is worth noting that the increase in disparity in the Northern American fossil record was ``almost entirely due to an influx of multituberculate and archaic ungulate immigrants'' \citep{Wilson2013} 
% NC: Never quote sections. Rewrite the statement and cite.
thus suggesting that global diversity was likely to be higher (i.e. where the immigrants originated from).
% NC remove this (i.e.) Either write it out in full or don't include it.
...
\cite{Slater2012MEE} showed evidence for a change in the mode of body-mass evolution at the K-Pg boundary, with a shift between constraint body mass evolution % NC: What is constraint body mass evolution
during the Cretaceous (Ornstein-–Uhlenbeck process) to an unconstrained evolution (Brownian Motion) right after the K-Pg event.
% NC: Unconstrained is probably not the right word here.
These results support a ``K-Pg release and radiate'' model for body mass evolution suggesting a strong effect of the K-Pg boundary on mammalian body mass evolution \citep{Slater2012MEE}. % NC: how does it change? How does this relate to our hypotheses. TG: is that better? NC: A bit, the whole thing needs a lot of thought.
In the present study, we do not find evidence for any effect of the K-Pg boundary and we are confident that our results are robust as we also find the same lack of effect from the independent Eutheria dataset \citep[i.e.][]{beckancient2014}.
...
In addition, variation in morphology and variation in body mass are two different aspects of diversity and may be decoupled in the same way as taxonomic diversity is decoupled from disparity \citep{slaterCetacean,ruta2013,hopkinsdecoupling2013}, thus it is possible that body-mass was influenced by the K-Pg extinction event, while overall disparity was not.
...
The disparity of each subsample is not independent of disparity in other subsamples in three ways: (1) each subsamples are time dependent; (2) each taxa in the subsamples are phylogeneticaly dependant and (3) the disparity variance and mean is calculated from bootstraps pseudo-replications.
% NC: Needs a clearer explanation of each point. Why are they autocorrelated?
...
% NC: Again you need to explain properly
Currently, however, this method is still widely used in disparity analyses for lack of a better alternative \citep[e.g.][]{anderson2012using,zelditch2012geometric,smith2014joined} and can lead to interpretable results when using p-value corrections \citep[e.g. Holm-Bonferonni][]{holm1979simple}. % NC: What do you mean by interpretable results? Does Bonferonni really fix this? I doubt it.
...
However, by doing so, we included dimensions of the cladisto-space with near zero variance and range (Mammaliaformes: variance = $2\times10^{-14}$, range = $7.31\times10^{-7}$; Eutheria: variance = $1.15\times10^{-15}$, range = $3.33\times10^{-7}$). % NC: What are these the range and variance of? One of the eigenvectors?
...
This metric is not affected by problems with using the last dimensions of the cladisto-space (see above) and also has several other advantages over other metrics.
For example, this metric has no special statistical assumptions, % and appears to be less coupled with taxonomic diversity % NC: Need to explain where you got this from - your supp analyses? TG: yeah but it's not thorough. It just looks decoupled visually, didn't tested anything. 
% NC: OK then I don't think you can say this.
...
% NC: Sheer does not mean what you think it does!
