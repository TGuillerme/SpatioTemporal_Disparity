...
% TG: PLoS Biology requires a 50 characters short title. How about that one?
\bigskip
\medskip
\begin{center}
...
Groups that survive mass extinction events can undergo evolutionary radiation due to reduced competition and/or to newly available ecological niches.
The radiation of mammals after Cretaceous-Paleogene (K-Pg) event that caused the extinction of non-avian dinosaurs (66 million years ago) is a textbook example illustrating such turn-over events.
...
The core of this debate lies in the different data used to investigate this question: either morphological data used by palaeontologists or molecular data used by neontologists.
...
Using both trees we found no significant change in disparity before and after the K-Pg boundary, and this under both evolutionary models.
...
%TG: currently 286 words. No limit for PLoS Biology but maybe we want to keep it under 300?
\end{abstract}
...
% §2 - What does it mean - TG: a posteriori comment: maybe we can skip the §1 since it says exactly the same as §2...
...
Our results suggest that mammals morphology didn't diversified in response to the K-Pg event.
In fact, their diversification appeared to have started prior to the K-Pg extinction \citep[Fig \ref{fig:Fig_Raw_results} and Table \ref{tab:Tab_results} and ][]{meredithimpacts2011,dosReis2014,Close2015,Lee2015R759}.
...
This might be due to a late lag effect after the K-Pg event ($\sim$21 Ma afterwards) or to an effect of the PETM \citep[$\sim$ 11 Ma afterwards;][]{bininda2007delayed}.
Nevertheless, this significant increase in disparity is only detected at 45 Ma but not afterwards neither under gradual evolutionary model.
...
In fact, the 45 Ma time slice %TG: or subsample? Maybe standardize it by "subsample = the tips and nodes sampled a time X (i.e. tips and nodes are rarely present at time X)" and "time slice = the true biological instant photography of the species at time X"
...
These differences might be related to several methodological difference between this study and \cite{Wilson2013}.
...
% TG: A funny note (not tested in this paper): I more and more start to believe that the K-Pg extinction is a global marine extinction but on the continents, it's only a Northern-American (and also maybe European) extinction...
...
\cite{Slater2012MEE} found good support for a shift of mode of body mass evolution (from an Ornstein--Uhlenbeck to a Brownian Motion) %TG: or "(a significant increase in cumulative body mass variance after the K-Pg event)"
...
Body mass and disparity might be decoupled in a similar way than taxonomic diversity and disparity \citep[e.g.][]{slaterCetacean,ruta2013,hopkinsdecoupling2013} since disparity does not rely on size but rather on discrete morphological features.
...
Yet we remain confident of our results because we recovered the same pattern from two independent datasets \citep{Slater2012MEE,beckancient2014}.
