...
%TG: title was "Mammalian morphological diversity does not increase in response to the Cretaceous-Paleogene mass extinction."
\bigskip
...
\noindent All data are available on Figshare (\url{http://dx.doi.org/10.6084/m9.figshare.1539545}) and the code for reproducing the analyses is available on GitHub (\url{http://github.com/TGuillerme/SpatioTemporal_Disparity}). % NC: Is this what Evolution requests?
...
Here we propose a new methodological approach to this debate, looking at living and fossil taxa simultaneously.

We investigated the effect of the K-Pg extinction event on mammalian morphological diversity (disparity) using two Total Evidence tip-dated phylogenies of Mammaliaformes and Eutheria.
Using a novel, continuous time-slicing method for measuring changes in disparity-through-time, we found no significant change in total disparity before and after the K-Pg boundary, under either a gradual or punctuated model of evolution.
This suggests that the extinctions at the end of the Cretaceous did not affect mammalian evolution into different morphologies. 
...
We can instead use morphological diversity, also known as disparity \citep[e.g.][]{Wills1994,Erwin2007,Hughes20082013}, as a way to quantify changes in mammalian morphology that should relate to the ecology of the species (although morphology does not always correlate with ecology \citep[e.g.][]{anderson2012using}).
%TG: I've added:
...
% NC: This seemed like a really long explanation. I think either just talk about morphology (and alter this whole paragraph) or don't draw too much attention to it.
...
% NC: Nevertheless does not mean what you think it does. Stick with words you know in manuscripts, remember clear writing should be kept simple.
...
Note that because of the obvious difficulties of representing multidimensional objects (i.e. respectively 177 and 201 dimensions for the Mammaliaformes and Eutheria datasets %NC: Is this important? ; see figure S3 for the cumulative variance per dimension), 
we do not represent any subset of dimensions of each dataset graphically \citep[c.f.][]{Foote01071994,Wesley-Hunt2005} as our analyses use all the dimensions and are thus impossible to graph. %but rather analyse the changes in the cladisto-space by using a summary metric of all the dimensions (i.e. disparity, see below). %The 2D representation of the first 9 pairs of dimensions of each cladisto-space is given in the supplementary figures S4 and S5 for illustrating the problem of representing a cladisto-space in 2 dimensions (note that the same problem applies with 3D).

% NC: I'm not sure all this is necessary. Seems like a lot to defend not putting in a figure. This is the kind of thing I'd put in a response letter but not a manuscript, especially since it is not going back to Current Biol. Why add S3? How does that help the argument?
...
Third, even though covariance between dimensions is minimized by creating dimensions that are orthogonal to each other, one should calculate the sum and the product of variances as the sum and product of variance-\textit{covariance} % NC: Not sure I follow but I will take your word for it but do you mean variance-covariance or just covariance?
...
% NC: OK I agree on the morphospace plots being stupid. But his other point is true. All you are measuring is TOTAL disparity. So on the plot below, disparity at X is the same as at Y. His point is that maybe you have (e.g.) bats pre KT and then dogs post KT. Same TOTAL disparity, but in a completely different region of morphospace. So technically what you are saying is not correct. I've tried to reword a bit here and in the abstract. It is just a question of calrifying exactly what we mean by disparity I think. You may need to change some "morphospaces" to cladistospaces.
...
Also, it is worth noting that cladistic data for living mammals is surprisingly rare \citep{MissingMammals}. % NC: Why is it worth noting exactly?
Besides, this might not cause a serious underestimation of disparity, at least for the Mammaliaformes, because their fossil diversity peaked during the late Cretaceous \citep[Campanian; 72.1--83.6 Ma;][]{Newham201432} and mammalian diversification rates declined throughout the Cenozoic \citep{Raia2012}.
...
% NC: Make sure to be careful with stuff like this. Steve is right - this error makes us look like idiots, even though you knew what you meant.
...
% NC: I think for now leave it here and see what happens at evolution.
