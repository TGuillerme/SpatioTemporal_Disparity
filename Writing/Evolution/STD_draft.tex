...
\noindent All data are available on Figshare (\url{http://dx.doi.org/10.6084/m9.figshare.1539545}) and the code for reproducing the analyses is available on GitHub (\url{http://github.com/TGuillerme/SpatioTemporal_Disparity}). % NC: Is this what Evolution requests? TG: Yap
...
In many studies it is measured as taxonomic diversity or species richness \citep{Stadler12042011,meredithimpacts2011,OLeary08022013}, but often the more interesting aspect of diversity is related to species morphological variations or the ecological niches they occupy \citep{Wesley-Hunt2005,Brusatte12092008,toljagictriassic-jurassic2013}, particularly if we want to make hypotheses about macroevolutionary processes \citep{Pearman2008149,OlsonRadiation,Losos2010,glor2010phylogenetic,benton2015}.
...
%TG: I've removed the ecology bits.
...
% TG: fair enough, I removed S3 thingy then.
...
Third, even though covariance between dimensions is minimized by creating dimensions that are orthogonal to each other, one should calculate the sum and the product of variances as the sum and product of variance-\textit{covariance} % NC: Not sure I follow but I will take your word for it but do you mean variance-covariance or just covariance? TG: var-covar.
...
%TG: agree. I think you made the point clear enough in the first paragraph. Thanks for that!
...
We used two of the biggest cladistic datasets (taxonomic wise) focusing on mammals around the K-Pg boundary, in fact they are within the same order of magnitude of one of the biggest mammalian cladistic dataset recently published \citep[][ with 157 coded primates]{ni2013oldest}.
Additionally, both datasets contain really few living mammals (respectively 22 and 14 for the Mammaliaformes and Eutheria datasets) which is due to the surprising rarity of available cladistic data for living mammals \citep{MissingMammals}. % NC: Why is it worth noting exactly? TG: changed to additionally
%Besides, this might not cause a serious underestimation of disparity, at least for the Mammaliaformes, because their fossil diversity peaked during the late Cretaceous \citep[Campanian; 72.1--83.6 Ma;][]{Newham201432} and fossil mammalian diversification rates declined throughout the Cenozoic \citep{Raia2012}.

%TG: I'm not sure about these statements any more. As in, I think they make sense (more fossil mammals in the past + declining fossil diversification rates in the Coenozoic) but it kind of doesn't fit here if we want to lower our claims (i.e. we don't want to say anything on diversification)
