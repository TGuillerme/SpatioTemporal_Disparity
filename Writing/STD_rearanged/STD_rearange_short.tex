...
\noindent {\normalsize \sc Thomas Guillerme$^1$$^,$$^*$, and Natalie Cooper$^3$}\\
\noindent {\small \it 
$^1$School of Natural Sciences, Trinity College Dublin, Dublin 2, Ireland.\\
%$^2$Trinity Centre for Biodiversity Research, Trinity College Dublin, Dublin 2, Ireland.\\ RM
...
Throughout history, life on Earth has suffered a series of mass extinction events resulting in drastic declines in global biodiversity (e.g. \cite{rennetime2013,Brusatte2015}). %\cite{RaupPT,BentonPT,rennetime2013,Brusatte2015}. % TG: or keep Raup and Benton (but takes two extra cites)
...
\subsection{Conclusion} TG: not sure where do we want to put that in the end
...
% For example, if a character had two observed states ($0$ and $1$) across all taxa, we attributed the multi-state ``$0$\&$1$" value to the taxon with missing data, representing an equal probability of being either $0$ or $1$. TO-SUP
% This allows the ancestral node of a taxon with missing data to be estimated with no assumptions other than that the taxon has one of the observed character states. TO-SUP
...
Although empirically inter-taxon distances are the same in a morphospace or a cladisto-space \cite{foth2012different,hetherington2015cladistic}, we prefer the term cladisto-space to make it clear that this space is estimated using cladistic data and not morphometric data.% and because both objects have slightly different properties. RM
% For example, because of its inherent combinatory properties, a cladisto-space is a finite theoretical object limited by the product of the number of character states, whereas a morphospace is an infinite theoretical object. RM
...
% For cladistic matrices, using this corrected distance is preferable to the raw Euclidean distance because of its ability to deal with discrete or/and ordinated characters as well as with missing data \cite{anderson2012using}. RM?
